...
sub get_user_favourites ($self, $id){
	my @ans;
	my $sth = $self->dbh->prepare('SELECT word.word, word.sense FROM user INNER JOIN favourite on user.role_id=? INNER JOIN word ON favourite.username=user.username');
	$sth->execute($id);
